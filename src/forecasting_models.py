...
def fit_moving_average_forecast(train: pd.Series, test: pd.Series, window: int = 3) -> ForecastResult:
    """Moving-average baseline forecast over the test horizon.

    Computes the mean of the last *window* training observations and predicts
    that constant value for every test step.  This non-recursive formulation
    matches the paper's simple-baseline design (Table IV) and yields the
    reported MAE/RMSE/MAPE figures for MA(3).
    """
    if window < 1:
        raise ValueError("window must be >= 1")
    train_vals = train.astype(float).values
    recent = train_vals[-window:] if len(train_vals) >= window else train_vals
    pred_val = float(np.mean(recent))
    pred_series = pd.Series(pred_val, index=test.index, name=f"ma_{window}_pred")
...
def fit_lstm_forecast(
    ...
    Architecture as described in the paper: single LSTM layer with 32 hidden
    units, dense output layer, Adam optimizer (lr=0.001), sliding window of
    6 months, trained for 100 epochs.
    """
    try:
    ...
    if denom < 1e-8:
        raise ValueError(
            "Training series has near-zero variance; cannot scale for LSTM training."
        )
