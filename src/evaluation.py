...
def run_adf_test(series: pd.Series) -> dict[str, float]:
    """Run the Augmented Dickey-Fuller stationarity test on *series*.

    Returns a dict with keys ``adf_statistic``, ``p_value``, and
    ``is_stationary`` (True when p_value < 0.05).

    Paper result (Section V.B): ADF statistic = -5.4796, p-value < 0.001,
    confirming stationarity of the processed price series.
    """
    try:
        from statsmodels.tsa.stattools import adfuller
    except ImportError as exc:
        raise RuntimeError(
            "statsmodels is required for the ADF test. Install with `pip install statsmodels`."
        ) from exc

    result = adfuller(series.dropna().astype(float), autolag="AIC")
...
def build_model_comparison_table(
    ...
    MAE and RMSE are reported in the same units as the input series
    (million VND/m² when using the public datasets).  MAPE is a percentage.
    Values are rounded to 2 decimal places to match paper formatting.
    """
    rows = []
...
def build_baseline_comparison_table(
    ...
    """Build baseline comparison table with paper schema (Table IV).

    Includes naive, moving-average, ARIMA, Prophet, and LSTM.  All metric
    values are in the same units as the input series (million VND/m²).
    """
    rows = []
